...
class APIHybridizationProbability(APIBase):
    ...
    def __init__(self, ai_filter_path: str | None = None) -> None:
        ...
        if ai_filter_path is None:
            ...
            self.ai_filter_path = os.path.join(
                package_path, "pretrained_models", "hybridization_probability.pt"
            )
        else:
        ...
        self.inverse_predictions = lambda predictions: 10 ** (
            predictions * loaded_model["hyperparameters"]["dataset"]["std"]
            + loaded_model["hyperparameters"]["dataset"]["mean"]
        )
