...
from oligo_designer_toolsuite.oligo_property_filter import HardMaskedSequenceFilter, PropertyFilter
...
def duplexing_log_scores(oligo: str, off_target: str, model: nupack.Model, concentration: float) -> float:
    ...
    t = nupack.Tube(
        strands={
            oligo_strand: concentration,
            on_target_strand: concentration,
            off_target_strand: concentration,
        },
        name="t",
    ...
    off_target_perc = tube_concentrations[nupack.Complex(strands=[oligo_strand, off_target_strand])] / (
        tube_concentrations[nupack.Complex(strands=[oligo_strand, off_target_strand])]
        + tube_concentrations[nupack.Complex(strands=[oligo_strand, on_target_strand])]
    )
...
def generate_off_targets(sequence: Seq, config) -> list[Tuple[str, str, int, float]]:
    ...
    off_target_regions = [
        (
            ...
            str(sequence),
            0,
            duplexing_log_scores(str(sequence), str(sequence), model, config["concentration"]),
        )
    ...
    for i in range(1, config["max_mutations"] + 1):  # nr of mutations
        for _ in range(1, config["n_mutations_per_type"] + 1):  # nr of mutations for mutation class
            ...
            off_target_regions.append(
                (
                    str(sequence),
                    str(target),
                    i,
                    duplexing_log_scores(str(sequence), str(target), model, config["concentration"]),
                )
            )
    ...
    for i in range(1, config["max_bulges_size"] + 1):  # nr of mutations
        for _ in range(1, config["n_mutations_per_type"] + 1):
            ...
            for _ in range(i):
                ...
                new_sequence.insert(
                    insertion_point, "-"
                )  # generate to have a correct alignement with of the sequnces (- with be encoded as a 0 vector)
            off_target_regions.append(
                (
                    str(new_sequence),
                    str(target),
            ...
            for _ in range(i):
                new_target.insert(
                    deletion_point, "-"
                )  # generate to have a correct alignement with of the sequnces
            off_target_regions.append(
                (
                    str(sequence),
                    str(new_target),
...
def generate_dataset(alignments: list):
    dataset = pd.DataFrame(
        index=list(range(len(alignments))),
        columns=[
            "query_sequence",
            "query_length",
            "query_GC_content",
...
def main():
    ...
    size = (
        config["n_oligos"]
        * config["n_mutations_per_type"]
        * (config["max_mutations"] + config["max_bulges_size"])
    )
    ...
    oligo_database = property_filter.apply(
        oligo_database=oligo_database, n_jobs=config["n_jobs"], sequence_type="oligo"
    )
    ...
    oligos_train = [
        oligo_database.database[gene][oligo_id]["oligo"]
        for gene in genes_train
        for oligo_id in oligo_database.database[gene]
    ]
    oligos_validation = [
        oligo_database.database[gene][oligo_id]["oligo"]
        for gene in genes_validation
        for oligo_id in oligo_database.database[gene]
    ]
    oligos_test = [
        oligo_database.database[gene][oligo_id]["oligo"]
    ...
    oligos_validation = random.sample(
        population=oligos_validation, k=min(len(oligos_validation), sample_validation * 10)
    )
    ...
    if (
        len(oligos_train) < sample_train
        or len(oligos_validation) < sample_validation
        or len(oligos_test) < sample_test
    ):
    ...
    oligos_validation = random.sample(
        population=oligos_validation, k=min(len(oligos_validation), sample_validation)
    )
    oligos_test = random.sample(population=oligos_test, k=min(len(oligos_test), sample_test))
    logging.info(
        f"Sampled {len(oligos_train)} oligos for training, {len(oligos_validation)} oligos for validation, and {len(oligos_test)} oligos for testing"
    )
    ...
    train_alignments = [
        alignment for oligo_alignments in train_alignments for alignment in oligo_alignments
    ]  # flatten the returned structure
    # validation
    ...
    validation_alignments = [
        alignment for oligo_alignments in validation_alignments for alignment in oligo_alignments
    ]  # flatten the returned structure
    # # test
    ...
    test_alignments = [
        alignment for oligo_alignments in test_alignments for alignment in oligo_alignments
    ]  # flatten the returned structure
    ...
    logging.info(
        f"Dataset created and stored at: \n\t - {file_train},\n\t - {file_validation}, \n\t - {file_test}."
    )
