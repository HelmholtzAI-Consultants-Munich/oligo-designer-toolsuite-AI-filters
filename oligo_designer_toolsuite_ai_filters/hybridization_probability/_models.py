...
class OligoMLP(nn.Module):
    ...
    def __init__(
        self, input_size: int, hidden_size: int, n_layers: int, act_function: str = "relu", dropout: float = 0
    ) -> None:
        """Base MLP class that geenrates the duplexing score predictions from the One-Hot encodings of the two sequences and additional features sucha as the GC content and the melting temperature difference.
...
class OligoRNN(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        act_function = parse_act_function(act_function)
        hidden_size_shared = hidden_size + (
            bidirectional * hidden_size
        )  # if bidirectional the output tensor has doubled length
        self.pool = pool
        self.n_layers = n_layers

        self.encoding_MLP = [
            torch.nn.Linear(in_features=input_size, out_features=hidden_size_ecoder),
            act_function,
            nn.Dropout(p=dropout),
        ]
        for _ in range(n_layers_encoder - 1):
            self.encoding_MLP.extend(
                [
                    torch.nn.Linear(in_features=hidden_size_ecoder, out_features=hidden_size_ecoder),
                    act_function,
                    nn.Dropout(p=dropout),
                ]
            )
        self.encoding_MLP.append(
            torch.nn.Linear(in_features=hidden_size_ecoder, out_features=hidden_size_ecoder)
        )
        self.encoding_MLP = nn.Sequential(*self.encoding_MLP)

        self.recurrent_block = torch.nn.RNN(
            input_size=hidden_size_ecoder,
        ...
        for _ in range(n_layers_mlp):
            self.shared_MLP.extend(
                [
                    torch.nn.Linear(in_features=hidden_size_shared, out_features=hidden_size_shared),
                    act_function,
                    nn.Dropout(p=dropout),
                ]
            )
        ...
        for _ in range(n_layers_mlp):
            self.final_MLP.extend(
                [
                    torch.nn.Linear(
                        in_features=hidden_size_shared + features_size,
                        out_features=hidden_size_shared + features_size,
                    ),
                    act_function,
                    nn.Dropout(p=dropout),
                ]
    ...
    def forward(self, sequences: rnn.PackedSequence, features: torch.Tensor):
        ...
        encoded_sequences = rnn.PackedSequence(
            data=encoded_sequences, batch_sizes=sequences.batch_sizes.to("cpu")
        )
        ...
        processed_hidden_states = rnn.PackedSequence(
            data=processed_hidden_states, batch_sizes=hidden_states.batch_sizes.to("cpu")
        )
        unpacked_hidden_state, _ = rnn.pad_packed_sequence(processed_hidden_states, batch_first=True)
        # pool the hidden states
        if self.pool == "max":
            pool_function = lambda h: torch.max(input=h, dim=0, keepdim=True)[
                0
            ]  # the output is a tuple of tensors
        elif self.pool == "min":
...
class OligoLSTM(OligoRNN):
    ...
    def __init__(
        ...
        self.recurrent_block = torch.nn.LSTM(
            input_size=hidden_size_ecoder,
            hidden_size=hidden_size,
            num_layers=n_layers,
            dropout=dropout,
