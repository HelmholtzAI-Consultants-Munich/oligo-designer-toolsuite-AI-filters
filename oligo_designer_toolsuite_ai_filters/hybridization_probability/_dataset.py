    ...
    def __init__(self, dataset: pd.DataFrame) -> None:
        ...
        for i, row in dataset.iterrows():
            ...
            for j in range(len(query)):
                encoding[j, :] = torch.tensor(
                    self.encode_nt(query[j]) + self.encode_nt(target[j]), dtype=torch.double
                )
...
def pack_collate(batch: list) -> Tuple[rnn.PackedSequence, torch.Tensor, torch.Tensor]:
    ...
    return (
        rnn.pack_padded_sequence(input=padded_sequences, lengths=lengths, batch_first=True),
        features[perm_idx, :],
        labels[perm_idx],
    )  # reorder according to the original ordering
